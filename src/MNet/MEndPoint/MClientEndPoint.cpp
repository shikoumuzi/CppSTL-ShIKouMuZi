...
namespace MUZI::net
{
	...
	MClientEndPoint::MClientEndPoint(String raw_ip_address, Port port, int& ec) :m_data(new MClientEndPointData())
	{
		...
		{
			ec = MERROR::MNET_IPADDRESS_FORMAT_CANNOT_ANALIZE;
			fprintf(stderr, "Failed to parse the IP address, Error Code = %d, Message is %s", MERROR::MNET_IPADDRESS_FORMAT_CANNOT_ANALIZE, error_code.message().c_str());
